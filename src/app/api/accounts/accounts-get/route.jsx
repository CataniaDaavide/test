...
export async function POST(req) {
  ...
  try {
    ...
    const filter = { userId: _id, status:"A" };

    const accounts = await accountsModel.find(filter);

    // dataResponse.result = true;
    // rtn.response = dataResponse;
    return new NextResponse(JSON.stringify({ accounts: accounts }), {
      status: 200,
    });
  } catch (error) {
    rtn.error =
      error.message.toString() + " on endpoint:/api/categories/categorie-get";
    rtn.response = "";
    return new NextResponse(JSON.stringify(rtn), { status: 500 });
  }
}
