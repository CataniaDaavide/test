...
const inputVariants = cva(
  "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground border w-full min-w-0 rounded-md px-3 text-sm shadow-xs transition-all outline-none disabled:pointer-events-none disabled:opacity-50 [&>svg]:pointer-events-none",
  {
    variants: {
      variant: {
        default:
          "bg-card text-primary hover:bg-card/90 focus-visible:ring-ring/50 focus-visible:border-ring",
        destructive:
        ...
        ghost:
          "bg-transparent hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 focus-visible:ring-ring/50 focus-visible:border-ring",
        link: "bg-transparent text-primary underline-offset-4 hover:underline focus-visible:ring-ring/50 focus-visible:border-ring",
        error:
          "border-destructive text-destructive focus-visible:ring-destructive/50 focus-visible:border-destructive",
      },
      size: {
        default: "py-2",
        ...
        <div className="relative w-full flex items-center">
          ...
          {action ? (
            ...
            <>
              ...
              {/* Date Picker */}
              {type === "date" && (
                <DateAction value={value} onChange={onChange} />
              )}
            </>
          )}
        </div>
